...
def job_skill(job_id):
    cursor.execute("SELECT skill_id,skill_job_id,replace(replace(replace(skill_name, char(34), ''), \
        char(148), ''), char(147), '')AS skill_name,skill_level,skill_delete FROM job_skills \
        WHERE skill_job_id='" +job_id+ "' AND skill_delete='NO'")
    return cursor.fetchall()
